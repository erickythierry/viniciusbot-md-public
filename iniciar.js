...
(async () => {
    ...
    var loadState = () => {
        var state = undefined;
        try {
            var sessão = JSON.parse(fs.readFileSync('./session.json', { encoding: 'utf-8' }), baileys.BufferJSON.reviver);
            state = {
                creds: sessão.creds,
            ...
            try {
                ...
                */
                ...
                const sendImage = (img, mensagem) => {
                    client.sendMessage(from, { image: img, caption: mensagem, jpegThumbnail: img }, {
                        quoted: msg
                    });
                }
                ...
                if (isCmd) {
                    switch (comando) {
                        case 'msg':
                            client.sendMessage(from, { text: JSON.stringify(msg, null, '\t'), quoted: msg }, { quoted: msg })
                            break
                        case 'sendtext':
                            reply(`Text Here`)
                            break
